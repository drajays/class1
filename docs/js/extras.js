...
const Shop = {
  render() {
    ...
    grid.innerHTML = SHOP_ITEMS.map((item) => {
      const owned = (p.stickers || []).includes(item.id);
      return `
        <div class="shop-item ${owned ? 'owned' : ''}">
          <div class="shop-emoji">${item.emoji}</div>
          <b>${item.name}</b>
          <p>${owned ? '✅ Owned' : `🪙 ${item.cost}`}</p>
          ${owned ? '' : `<button class="btn-fun green btn-big shop-buy" data-id="${item.id}">Buy</button>`}
        </div>`;
    }).join('');
    grid.querySelectorAll('.shop-buy').forEach((btn) => {
      btn.addEventListener('click', () => this.buy(btn.dataset.id));
    });
    Speech.navSay(NAV_PROMPTS.shop);
  },

  buy(id) {
    const result = Store.buySticker(App.playerId, id);
    if (result.ok) {
      Sounds.correct();
      Rewards.confetti(20);
...
const Features = {
  render() {
    ...
    list.innerHTML = FEATURES_LIST.map((f, i) => `
      <div class="feature-card">
        <span class="feature-num">${i + 1}</span>
        <div><b>${f.title}</b><p>${f.text}</p></div>
      </div>
    `).join('');
    Speech.navSay(NAV_PROMPTS.features);
  },
};

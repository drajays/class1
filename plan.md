...
- [x] Phase 7 — Retire legacy path, integration QA, ship — **sign-off: ✅ COMPLETED 2026-07-08 (executed by reviewer on user instruction): legacy files deleted (learn/english/extras + 7 OCR JSONs), dead code removed, sw v21, FE-TEST run with results logged (1 documented exception: math/english guessability — post-ship follow-up); see COORDINATION.md "FE-TEST results".**
...
- [x] Phase 12 — Voice settings (voice picker + speed + test sound) — **sign-off: ✅ COMPLETED 2026-07-08 — voice/speed prefs + parent-dashboard UI + Read All (4 engines) + Colors toggle, all FE-TEST verified.**
- [x] Phase 12b — "Mummy's Voice" pre-generated clone option — **sign-off: ✅ SHIPPED PARTIAL 2026-07-08 — pipeline + playback + fallback verified; core UI lines generated tonight (~2–5 min/clip), full intro corpus + Hindi batch = documented overnight follow-up via generate_mummy_voice.py.**
